...
当前页面请求id：${this.reqId}${this.idFromServer ? ' (来自服务端)' : ' (自动生成)'}`;
    ...
    _storageUnsendData() {
        window.onunload = () => {
            ...
            if (this.queue.length) {
                if (navigator.sendBeacon && navigator.sendBeacon(this.url, JSON.stringify(this.queue))) {
    ...
    _printConsole(time, level, ...args) {
        if (console) {
            if (this.stylize) {
                console[level](`%c[${this._getTimeString(time)}] [${level.toUpperCase()}] -`, `color: ${Lajax.colorEnum[level]}`, ...args);
            } else {
                console[level](...args);
            }
        }
